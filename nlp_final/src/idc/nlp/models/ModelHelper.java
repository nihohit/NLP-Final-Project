...
public class ModelHelper {
	...
	public Problem createProblem(SongCollection metalSongs, SongCollection popSongs) {
		...
		p.l = metalSongs.values.length + popSongs.values.length;
		
		//the number of total features in input
		p.n = LyricsData.size();
		
		//the feature nodes of the training set
		p.x = convertTrainingSetToFeatureNodes(metalSongs.values, popSongs.values);
		
		//the target values of the known feature nodes
		p.y = createTargetValuesArray(metalSongs.values.length,	popSongs.values.length);
		return p;
	}
	 ...
	 * @param popSongs an array of pop songs in a form of <code>Song</code>.
	...
	public FeatureNode[][] convertTrainingSetToFeatureNodes(Song[] metalSongs,
			Song[] popSongs) {
		FeatureNode[][] temp = new FeatureNode[metalSongs.length + popSongs.length][];
		int row = 0;
		...
		for (int i = 0; i < popSongs.length; i++, row++) {
			Song app = popSongs[i];
